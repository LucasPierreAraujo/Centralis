  ...
  const carregarAta = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setLivro(ata.livro);
        setTipoSessao(ata.tipoSessao || 'MAGNA');
        ...
        setLocal(ata.local || '');
        setUsarAssinaturas(ata.usarAssinaturas ?? false);
        ...
        if (raw.startsWith('[')) {
          try {
            setCoberturasTemplo(JSON.parse(raw));
          } catch {
            setCoberturasTemplo([]);
          }
        } else if (raw) {
  ...
  const salvarAta = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Erro ao atualizar ata: ${error.error}`);
      }
    } catch (error) {
      ...
      <header className="bg-blue-900 text-white p-3 md:p-4 shadow-lg">
        <div className="w-full flex justify-between items-center">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div>
              <h1 className="text-lg md:text-2xl font-bold">Editar Ata</h1>
              <p className="text-xs sm:text-sm text-blue-200 hidden sm:block">Alterar dados da ata {numeroAta}</p>
            </div>
          </div>
          <button
            onClick={salvarAta}
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        <Breadcrumbs
          items={[
            ...
            { label: `Ata ${numeroAta}`, href: `/atas/${params.id}/visualizar` },
            { label: 'Editar' }
          ]}
        />
        {/* Dados Básicos */}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {coberturasTemplo.length === 0 && (
                <p className="text-gray-400 text-sm py-2">Nenhuma cobertura registrada.</p>
              )}
              <div className="space-y-3">
                {coberturasTemplo.map((cob, idx) => (
                  <div key={idx} className="border-2 border-gray-200 rounded p-3">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-semibold text-gray-600">Cobertura {idx + 1}</span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <div>
                        ...
                        <select
                          value={cob.tipo}
                          onChange={(e) => atualizarCobertura(idx, 'tipo', e.target.value)}
                          className="w-full border-2 border-gray-300 rounded px-3 py-2 text-gray-900"
                        >
                      ...
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">Irmão</label>
                        <input
                          list="membros-cobertura-templo"
                          value={cob.membro}
